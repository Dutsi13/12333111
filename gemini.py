...
async def adm_stats(message: Message, command: CommandObject):
    ...
    try:
        ...
        active_list = "\n".join([f"• `{r[0]}` (до {time.strftime('%H:%M %d.%m', time.localtime(r[1]))})" for r in active_rents]) or "Нет активных"

        cur.execute('SELECT phone, duration, cost, date FROM rent_history WHERE user_id = ? ORDER BY id DESC LIMIT 5', (uid,))
        h_rents = cur.fetchall()
        history_rent_list = "\n".join([f"• `{h[0]}` | {h[1]} мин | ${h[2]} ({h[3]})" for h in h_rents]) or "История пуста"
...
async def adm_del_acc(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID: return
    if not command.args:
        return await message.answer("⚠️ Формат: `/delacc +79991234567`")
    ...
    if os.path.exists(session_path):
        try:
            ...
            await message.answer(f"✅ Аккаунт `{phone}` и файл сессии удалены.")
...
async def manage_acc(call: types.CallbackQuery):
    ...
    await call.message.edit_caption(caption=f"📱 {p}\nСтатус: {'🔥 РАБОТАЕТ' if res[0] else '💤 ПАУЗА'}",
                                    reply_markup=kb.adjust(2, 2, 2, 1).as_markup())
...
async def topup_init(call: types.CallbackQuery, state: FSMContext):
    ...
    if method == 'stars':
        ...
        await call.message.answer("Введите сумму в USD для пополнения через CryptoPay:", reply_markup=back_kb().as_markup())
...
async def create_pay(message: Message, state: FSMContext):
    ...
    if data['method'] == 'stars':
        ...
        await message.answer_invoice(
            ...
            payload=f"paystars_{usd_equivalent}",
            currency="XTR",
...
async def success_pay(m: Message):
    # Извлекаем USD эквивалент из payload
